# portfolio_cloud.py
# Stabil portföljrapport via GitHub Actions (HTML + text)
...
# - Robust mot None från yfinance (kraschar inte)
...
# =========================================================
# 2) EMAIL / SMTP – bakåtkompatibel med olika secret-namn
# =========================================================
...
def _fmt_price(x: Any) -> str:
    # Kurs med 2 decimaler (svensk komma)
    try:
...
# =========================================================
# 5) Robust pris-hämtning
# =========================================================
def fetch_quote(symbol: str) -> dict[str, float | str | None]:
    """
    Returnerar price, prev_close och currency.
    Robust: fast_info -> info -> history
    """
    t = yf.Ticker(symbol)
    ...
    return {"price": price, "prev_close": prev_close, "currency": currency or "SEK"}
...
def compute_portfolio(holdings: list[dict[str, Any]]):
    ...
    for h in holdings:
        ...
        price = q["price"]
        prev = q["prev_close"]
        currency = str(q.get("currency") or "SEK").upper()

        if price is None or prev is None:
            print(f"VARNING: saknar pris/prev_close för {name} ({symbol})", file=sys.stderr)
            continue
...
def build_html(rows, total_value, total_change, total_pct, title, timestamp):
    ...
    for r in rows:
        ...
        <tr>
          ...
          <td style="padding:7px 5px;border-bottom:1px solid #e7e7e7;{num_cell}color:{color};font-weight:600">{_fmt_pct(r['pct'])}</td>
        </tr>
        """
    ...
    <div style="font-family:Arial,sans-serif;max-width:860px;margin:0 auto;padding:6px 6px">
      ...
      <div style="overflow-x:auto;-webkit-overflow-scrolling:touch;border-radius:10px;margin-left:-8px;margin-right:-8px">
        <table width="100%" cellspacing="0" cellpadding="0"
               style="border-collapse:collapse;border:1px solid #e7e7e7;border-radius:10px;overflow:hidden;table-layout:fixed;font-size:11.5px">
          <colgroup>
            <col style="width:27%">
            <col style="width:10%">
            <col style="width:17%">
          ...
          <thead>
            <tr style="background:#f6f7f9">
              ...
              <th align="right" style="padding:7px 5px;border-bottom:1px solid #e7e7e7;font-size:11px;color:#333">%</th>
            </tr>
          </thead>
          <tbody>
...
def build_text(rows, total_value, total_change, total_pct, title, timestamp):
    ...
    for r in sorted(rows, key=lambda r: r["value"], reverse=True):
        lines.append(
            f"{r['name']} | {_fmt_shares(r['shares'])} | {_fmt_price(r['price_sek'])} | "
            f"{_fmt_int(r['value'])} | {_fmt_int(r['change'])} | {_fmt_pct(r['pct'])}"
        )
    return "\n".join(lines)
