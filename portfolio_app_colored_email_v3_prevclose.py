...
GMAIL_APP_PASSWORD = "tshozrsrxvhoyadv"  # app-lösenord (utan mellanslag)
...
def get_fx_to_sek(currency: str) -> tuple[float, float]:
    """Returnerar (fx_now, fx_prev_close) för currency→SEK."""
...
def fetch_quote(symbol: str) -> tuple[float, float, str]:
    """Returnerar (last_price, prev_close_price, currency)."""
    t = yf.Ticker(symbol)
    info = t.fast_info or {}
...
def build_email_html(updated_at: str, total_value: float, total_day: float, rows: list[dict]) -> str:
    ...
    tr_html = []
    for r in rows:
        day_color = color_for(r["day_val"])  # NUMERISKT värde för färgsättning
        tr_html.append(
            f"""
            <tr>
...
class App(tk.Tk):
    ...
    def update_values(self):
        ...
        for h in HOLDINGS:
            ...
            price_local, prev_local, currency = fetch_quote(sym)
            currency = (currency or "SEK").upper()
            ...
            # Värde idag i SEK med både kursrörelse och FX-rörelse:
            price_sek = price_local * fx_now
            prev_sek = prev_local * fx_prev

            value_sek = price_sek * shares
            day_sek = (price_sek - prev_sek) * shares
